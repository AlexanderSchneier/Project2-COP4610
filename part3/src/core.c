...
char* queue_to_str(struct list_head* queue) {
    ...
    list_for_each_entry(patient_pet, queue, node) {
        if (patient_pet->pet != NULL) {
            buffer_idx += scnprintf(str_buffer + buffer_idx, Q_STR_BUF_SIZE - buffer_idx,
                                    "%c%d ", type_to_str[patient_pet->pet->type],
                                    patient_pet->pet->dest_floor);
        }
    }
...
char* generate_proc_string(void) {
    ...
    char* elevator_status = queue_to_str(&elevator->transport_queue.node);
    ...
    buffer_idx += scnprintf(buffer + buffer_idx, max_sz - buffer_idx,
                            ...
                            elevator->current_weight, elevator_status);

    kfree(elevator_status);

    for (int i = NUM_FLOORS - 1; i >= 0; --i) {
        char* floor_queue_str = queue_to_str(&floor_queues[i]);
        buffer_idx +=
            scnprintf(buffer + buffer_idx, max_sz - buffer_idx, "%s Floor %d: %d %s\n",
                      elevator->current_floor == (i + 1) ? "[*]" : "[ ]", i + 1,
                      get_queue_size(&floor_queues[i]), floor_queue_str);
